...
CONST INT LG_SMALL_FONT_TEXT_BOX_HEIGHT = 10;
CONST INT LG_MEDIUM_FONT_TEXT_BOX_HEIGHT = 10;
CONST INT LG_BIG_FONT_TEXT_BOX_HEIGHT = 7;

// corresponding font size
CONST INT LG_SMALL_FONT_SIZE = 7;
CONST INT LG_MEDIUM_FONT_SIZE = 7;
CONST INT LG_BIG_FONT_SIZE = 5;
CONST INT LG_CWTIMER_FONT_SIZE = 5.5;

// logical origin Y value for various font sizes
CONST INT LG_SMALL_FONT_LOGICAL_ORIGIN_Y = 0;
CONST INT LG_MEDIUM_FONT_LOGICAL_ORIGIN_Y = 0;
CONST INT LG_BIG_FONT_LOGICAL_ORIGIN_Y = 0;
...
// Font
CONST LPCTSTR LG_FONT = _T("Lucida Console");       //"Vrinda");//Microsoft Sans Serif"); //Vrinda
...
HANDLE CEzLcdPage::AddText(LGObjectType type, LGTextSize size, INT iAlignment, INT iMaxLengthPixels, INT iNumberOfLines)
{
    ...
    case LG_SCROLLING_TEXT:
        ...
        {
           // pStreamingText_->SetFontWeight(560);
	    ...
	    else if (LG_CWFONT == size)
        {
          //  pStreamingText_->SetFontWeight(FW_BOLD);
            iBoxHeight_ = LG_BIG_FONT_TEXT_BOX_HEIGHT;
            iFontSize_ = LG_CWTIMER_FONT_SIZE;
            iLocalOriginY_ = LG_BIG_FONT_LOGICAL_ORIGIN_Y;
        }
    ...
    case LG_STATIC_TEXT:
        ...
        {
			//pStaticText_->SetFontWeight(560);
...
HRESULT CEzLcdPage::SetText(HANDLE hHandle, LPCTSTR pText)
{
    ...
    if (NULL != pMyObject_)
    {
        assert(LG_STATIC_TEXT == pMyObject_->GetObjectType() || LG_SCROLLING_TEXT == pMyObject_->GetObjectType());
...
{
    assert(LG_FILLED == type || LG_CURSOR == type || LG_DOT_CURSOR == type);
    CLCDProgressBar *pProgressBar_ = new CLCDProgressBar();
    assert(NULL != pProgressBar_);
    ...
    ePROGRESS_STYLE eStyle = STYLE_FILLED;
	switch (type)
	{
		case LG_FILLED:
            eStyle = STYLE_FILLED;
...
HRESULT CEzLcdPage::SetProgressBarPosition(HANDLE hHandle, FLOAT fPercentage)
{
    ...
    if (NULL != pMyObject_)
    {
        assert(LG_PROGRESS_BAR == pMyObject_->GetObjectType());
...
HRESULT CEzLcdPage::SetProgressBarSize(HANDLE hHandle, INT iWidth, INT iHeight)
{
    ...
    assert(NULL != ppMyObject_);

    if (NULL != ppMyObject_)
    {
        assert(LG_PROGRESS_BAR == ppMyObject_->GetObjectType());
        // only allow this function for progress bars
